...
namespace GUIProject
{
    ...
    {
        public Rectangle(int X, int Y, int Width, int Height, Color C) : base(X, Y, C)
        {
            setWidth(Width);
            setHeight(Height);
        ...
        public override void Draw(Graphics g)
        {
            ...
            g.FillPolygon(new SolidBrush(getColor()), points);
        }
        ...
        {
            PointF point1 = new PointF(getX() + width / 2, getY() + height / 2);
            Matrix result = new Matrix();
            result.RotateAt(angle, point1);
