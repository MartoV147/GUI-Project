...
namespace GUIProject
{
    public partial class KnockOffPhotoShop : Form
    {
        ...
        public enum Item
        {
            Rectangle, Ellipse, Line, Triangle, Point, GroupShape, Envelope, Star
        }
        ...
        private void DrawingBoard1_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var item in DrawnShapesDrawingBoard1)
            {
                item.Draw(g);
            }
            ...
            //{
            //    ((GroupShape)SelectedShape).Draw(g);
            //}
        }
        ...
        private void ShowSelectedShapeProps()
        {
            ...
            else
            {
                do
                {
                    SelectedShape.setAngle(SelectedShape.getAngle() - 360);
                } while (SelectedShape.getAngle() > 360);
            }
            AngleTrackBar.Value = SelectedShape.getAngle();
            ...
            switch (SelectedShape.GetShapeType())
            {
                ...
                case ShapeTypeEnum.Envelope:
                    {
                        ShowHideLabels(false);
                        WidthTBox.Text = ((Envelope)SelectedShape).getWidth().ToString();
                        HeightTBox.Text = ((Envelope)SelectedShape).getHeight().ToString();
        ...
        public Shape CreateShape(int x1, int y1, int x2, int y2)
        {
            ...
            switch (chosenItem)
            {
                ...
                case Item.Envelope: shape = new Envelope(Math.Min(x1, x2), Math.Min(y1, y2), Math.Abs(x2 - x1), Math.Abs(y2 - y1), paintColor, Color.White); break;
                case Item.Star: shape = new StarShape(Math.Min(x1, x2), Math.Min(y1, y2), Math.Abs(x2 - x1), Math.Abs(y2 - y1), paintColor); break;
            }
            return shape;
        }
        ...
        private void DrawingBoard1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (chosenTool == Tool.MultipleSelect && multSelect)
            {
                ...
                {
                    selectRec = new Rectangle(Math.Min(x, e.X), Math.Min(y, e.Y), Math.Abs(e.X - x), Math.Abs(e.Y - y), Color.FromArgb(50, 255, 0, 0));
                }
                DrawingBoard1.Invalidate();
            }
            Invalidate();
        ...
        private void DrawingBoard1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (chosenTool == Tool.MultipleSelect)
            {
                selectRec = new Rectangle(Math.Min(x, e.X), Math.Min(y, e.Y), Math.Abs(e.X - x), Math.Abs(e.Y - y), Color.FromArgb(0, 0, 0, 0));
        ...
        {
            KnockOffPhotoShop knockOffPhotoShop = new KnockOffPhotoShop();
            knockOffPhotoShop.Visible = true;
        }
        #endregion
        ...
        private void AngleTrackBar_Scroll(object sender, EventArgs e)
        {
            ...
            {
                PointF[] points = null;
                ((GroupShape)SelectedShape).Rotate(SelectedShape.getAngle() + angle, points, SelectedShape.getCenter());
            }
            DrawingBoard1.Invalidate();
            angle = 0;
        }
        ...
        private void EnvelopeBtn_Click(object sender, EventArgs e)
        {
                chosenItem = Item.Envelope;
                label1.Visible = false;
                comboBox1.Visible = false;
        }
